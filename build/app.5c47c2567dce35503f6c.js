...
var a, account, animationOptions, c, child, d, delivery, device, deviceHeight, deviceWidth, fadeTransition, flow, footer, h, header, homepage, p, payment, product, r, s, sum, summary;
...
var homepage = class homepage {
  ...
  init() {
    ...
    self = this;
    this.s.sticky_get_started.parent = null;
    this.s.sticky_get_started.y = this.flow.y + 58;
    this.s.sticky_get_started.opacity = 0;
    this.s.sticky_get_started.centerX();
    this.s.sticky_get_started.states = {
      out: {
        y: this.flow.y + 58,
        opacity: 0
      },
    ...
    visible = false;
    this.flow.scroll.onMove(function() {
      if (self.flow.scroll.scrollY >= 700 && !visible) {
        visible = true;
        return self.s.sticky_get_started.stateCycle('in', 'out');
      } else if (self.flow.scroll.scrollY < 700 && visible) {
        visible = false;
        return self.s.sticky_get_started.stateCycle('out', 'in');
      } else {

      }
    });
    // body...
    this.assignTransition(this.s.get_started, self.s.Child_details, this.s.sticky_get_started);
    this.assignTransition(this.s.sticky_get_started, self.s.Child_details, this.s.sticky_get_started);
    this.assignTransition(this.s.get_started_code, self.s.Child_details_2, this.s.sticky_get_started);
    return this.assignTransition(this.s.sticky_get_started_code, self.s.Child_details_2, this.s.sticky_get_started);
  }
...
var account = class account {
  ...
  init() {
    var clickoff, i, len, link, results, sl;
    ...
    this.assignTransition(this.s.login, sl.s.Delivery_address);
    // Login
    this.s.have_an_account.on(Events.Click, function(event, layer) {
      return sl.flow.showOverlayCenter(sl.s.have_an_account_modal, {
        animate: true
      });
    });
    this.s.caret_account.states = {
      active: {
        rotation: 180
      },
      inactive: {
    ...
    this.assignActiveState(this.s.whitemask_account);
    this.assignActiveState(this.s.summary_unfolded);
    clickoff = [this.s.summary_title, this.s.caret_account, this.s.close_summary_unfolded, this.s.edit_order_summary_unfolded];
    results = [];
    for (i = 0, len = clickoff.length; i < len; i++) {
      link = clickoff[i];
      results.push((function(link) {
        return link.on(Events.Click, sl.dropdownEvent.bind(sl));
      })(link));
    }
    return results;
  }
  ...
  dropdownEvent(event, layer) {
    ...
    this.s.whitemask_account.visible = this.s.whitemask_account.visible ? false : true;
    this.s.whitemask_account.stateCycle('inactive', 'active');
    this.s.summary_unfolded.visible = this.s.summary_unfolded.visible ? false : true;
    return this.s.summary_unfolded.stateCycle('inactive', 'active');
  }
...
/***/ })
/******/ ]);
//# sourceMappingURL=app.5c47c2567dce35503f6c.js.map